"""
Page: Executive Overview with strategic risk blocks and KPI analytics.
"""
import html
...
def _render_strategic_portfolio(df_filtered, kpis: dict):
    ...
    agg_risk = float(df_filtered["risk_score"].mean()) if "risk_score" in df_filtered.columns else 0.0
    agg_risk = round(agg_risk, 1)
    risk_label, risk_color = _risk_state(agg_risk)
    ring_deg = max(8, min(360, int((agg_risk / 100) * 360)))

    delta = float(df_filtered["risk_score"].std()) if "risk_score" in df_filtered.columns else 0.0
    delta = round(delta, 1)

    sort_cols = [c for c in ["risk_score", "costo_exceso_mn"] if c in df_filtered.columns]
    if sort_cols:
        ascending = [False] * len(sort_cols)
        top_risk = df_filtered.sort_values(sort_cols, ascending=ascending).head(3)
    else:
        top_risk = df_filtered.head(3)
    interventions = []
    for _, row in top_risk.iterrows():
        score = float(row.get("risk_score") or 0)
        sev, dot = _risk_state(score)
        title = f"{row.get('cve_prod', 'SKU')} - {row.get('cse_prod', 'Familia')}"
        desc = (
            f"Score {score:.1f}/100 | Exceso ${float(row.get('costo_exceso_mn') or 0):,.0f} "
            f"| Cobertura {_safe(row.get('meses_inventario'), 0):.1f} meses"
        )
        interventions.append((html.escape(title), html.escape(desc), dot, sev))

    if not interventions:
        interventions.append(("Sin alertas", "No se detectaron intervenciones en el corte actual.", "#34D399", "Stable"))

    if "cse_prod" in df_filtered.columns:
        fam = (
            df_filtered.groupby("cse_prod", dropna=False)
            .agg(
                risk=("risk_score", "mean") if "risk_score" in df_filtered.columns else ("cve_prod", "count"),
                costo=("costo_exceso_mn", "sum") if "costo_exceso_mn" in df_filtered.columns else ("cve_prod", "count"),
                count=("cve_prod", "count"),
            )
            .reset_index()
    ...
    top_family = html.escape(str(kpis.get("top_family") or "N/D"))
    conc = (kpis.get("family_concentration") or 0) * 100
    zero_sales = kpis.get("zero_sales_pct") or 0
    potential = (kpis.get("total_excess_cost") or 0) * 0.05
    ...
    with left_col:
        st.markdown(
            f"""
            <div class='exec-card' style='padding:1.2rem 1.25rem; margin-bottom:0.95rem;'>
                <div style='display:flex; justify-content:space-between; gap:1rem; align-items:center;'>
                    <div style='max-width:68%;'>
                        ...
                        <div style='display:flex; align-items:baseline; gap:.5rem; margin-top:.35rem;'>
                            <div style='font-family:Montserrat,sans-serif; font-weight:900; font-size:3.25rem; line-height:1; color:#F8FBFF;'>{agg_risk:.0f}</div>
                    ...
                    <div style='position:relative; width:170px; height:170px;'>
                        ...
                        <div style='position:absolute; inset:18px; border-radius:50%; background:#0F151F; border:1px solid rgba(80,97,121,.6); display:flex; align-items:center; justify-content:center; flex-direction:column;'>
                            ...
                            <div style='font-size:.75rem; color:#AFC2DA;'>Portfolio</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    with right_col:
        ...
        for title, desc, dot_color, sev in interventions:
            st.markdown(
                f"""
            ...
            <div class='exec-card' style='padding:.95rem 1rem; margin-bottom:.95rem;'>
                ...
                <div style='padding:.72rem; border-radius:9px; background:rgba(56,189,248,.09); border:1px solid rgba(56,189,248,.22); margin-bottom:.55rem;'>
                    ...
                    <div style='font-size:.78rem; color:#EAF3FF; margin-top:.3rem;'>
                        La familia <strong>{top_family}</strong> concentra <strong>{conc:.1f}%</strong> del costo de exceso.
                    </div>
                </div>
