...
from config.settings import COLOR_DANGER, COLOR_PRIMARY, COLOR_SECONDARY, COLOR_SUCCESS, COLOR_WARNING
...
_LAYOUT_BASE = dict(
    ...
    margin=dict(l=10, r=10, t=24, b=16),
...
def chart_excess_trend(trend_df: pd.DataFrame):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            line=dict(color=COLOR_DANGER, width=2.4),
            marker=dict(size=7),
            fill="tozeroy",
...
def chart_top_products_bar(df: pd.DataFrame, value_col: str, label: str, n: int = 15):
    ...
    _apply_layout(fig, height=max(320, n * 28))
